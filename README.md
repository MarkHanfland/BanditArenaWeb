...
When using the cloud-hosted UI at `banditarena.com`, device API calls go from the browser to `http://localhost:8080` on the operator's machine. The Bandit Arena REST server must allow CORS from `https://banditarena.com` (configured in `http_server.cpp`). Rebuild and restart `bandit_arena.exe` after CORS changes.
...
- Local dev: `.env` or example amplify config; device pages still use `http://localhost:8080`
